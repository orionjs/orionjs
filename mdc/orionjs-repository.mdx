...
```typescript
import {createCollection, MongoFilter, Repository} from '@orion-js/mongodb'
...
```typescript 
import {schemaWithName, InferSchemaType, TypedId, typedId} from '@orion-js/schema'

export type CardId = TypedId<{prefix: 'crd'}>

export const CardSchema = schemaWithName('CardSchema', {
  _id: {type: typedId('card')},
  name: {type: String},
  // ... other properties
})
