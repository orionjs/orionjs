...
Pulse persists each `consumerGroup + topic` configuration. Increase the listener's integer
...
Pulse listeners keep queue state, leases, retries, and completed attempts atomically on their
delivery documents. Configure `maxConcurrency` on an event to override
...
Each array element contains its cleaned `params` and individual `context`, including `eventId`,
...
<Warning>
  Pulse no longer supports `events.pulse.changeStreams`. Event discovery always uses polling.
  Remove the legacy field before deploying this configuration; Echoes fails during
...
<Warning>
  ...
  Kafka and Pulse form two independent delivery streams, so they do not provide a shared global
  sequence. Use a coordinated publisher cutover after draining Kafka when cross-transport ordering
  matters.
</Warning>
