...
| `lockTime` | `number` | Override the global `defaultLockTime` for this job only. |
| `maxTries` | `number` | Override the global event `maxTries` for this job only. |
...
// One scheduler discovers jobs defined using the @Jobs() decorator and runs up to four at once.
const workersInstance = startWorkers({
  workersCount: 4, // Maximum concurrent executions (default: 4)
  ...
  maxTries: 10, // Optional. Applies only to event jobs
  maxTriesReachedRetentionMs: 7 * 24 * 60 * 60 * 1000, // Default: 1 week. Use null to keep forever
...
Each `startWorkers()` call creates one scheduler loop, regardless of `workersCount`. The scheduler
claims jobs atomically until its concurrency is full. When no job is pending, it performs at most one
unsuccessful polling query per `pollInterval`. `cooldownPeriod` is deprecated and ignored.
