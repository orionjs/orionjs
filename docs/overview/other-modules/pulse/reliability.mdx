...
public `createdAt` field remains available for application timestamps and delivery ordering.
...
Pulse configures `maxPoolSize: 5` by default. With 500 service replicas, that bounds the application
...
`workerCount`; use `pollIntervalMs` to balance idle query volume against delivery latency.

Discovery leadership uses its own renewable lease, configured with `discoveryLockTimeoutMs` and
defaulting to 10 seconds. This keeps leader failover prompt without forcing long-running handler
locks to use the same short timeout. Temporary discovery-query errors do not voluntarily abandon
the lease; fencing loss or expiry still allows another replica to take over safely.
