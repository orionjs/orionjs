...
Pulse writes the attempt as `pending` before it calls your handler. A renewable lease and UUIDv7 fencing token protect the attempt while it is running. Polling and reconciliation discover events and recover partial writes or dead workers; Pulse never opens MongoDB Change Streams.
