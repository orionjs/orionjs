...
Pulse is a typed pub/sub system for services that already operate MongoDB and do not want to run a separate event broker. It provides durable consumer groups, concurrent handlers, retries, execution history, and crash recovery.
...
| Delivery | Queue state, lease, retries, attempts, and outcome for one event and consumer group. |

Publishing stores one event. Each subscribed consumer group materializes its own delivery and
execution state. Replicas with the same consumer group compete for that delivery; services with
different consumer groups each receive it.
