...
Pulse is a typed pub/sub system for services that already operate MongoDB and do not want to run a separate event broker. It provides durable consumer groups, ordered or concurrent handlers, retries, execution history, and crash recovery.
...
| Delivery | Logical outcome of one event for one consumer group; execution v2 also embeds its attempt state. |
| History | Physical execution v1 attempts, including pending state, lease, result, duration, and error. |

Publishing stores one event. Each subscribed consumer group materializes its own delivery and
execution state. Version 1 stores attempts in history; opt-in version 2 stores a bounded attempt
window on the delivery. Replicas with the same consumer group compete for that delivery; services
with different consumer groups each receive it.
...
Pulse writes the attempt state as pending before it calls your handler. A renewable lease and UUIDv7
fencing token protect the attempt while it is running. Polling and recovery discover events and
recover partial writes or dead workers; Pulse never opens MongoDB Change Streams.
