...
## Embedded execution for high-throughput consumers

Unordered subscriptions can opt into execution version 2, which keeps the lease, retries, and
completed attempts atomically on the delivery document:
...
The default is `executionVersion: 1`. Version 2 rejects `ordered: true`; ordered consumers continue
using version 1.

Treat the switch as a persisted configuration change. First deploy a bridge-capable Pulse version
to every replica without changing `executionVersion`. In a later deployment, enable version 2 and
increase `configVersion`. Bridge workers execute both old and new delivery formats, so the existing
backlog drains while new work moves to the embedded path. Read the
[execution v2 rollout guide](/blog/pulse-embedded-execution) before enabling it in production.
...
`getSubscriptions()` is synchronous and reports local subscriptions. `unsubscribe()` is idempotent and preserves the durable MongoDB subscription. `close()` stops coordinator and worker execution, waits for background work to settle, and closes the MongoDB client.

If a handler itself calls and awaits `close()`, Pulse initiates shutdown but lets that call return
so the current attempt can finish and be acknowledged. Call `close()` again from the process
lifecycle owner when it needs to await complete shutdown.
