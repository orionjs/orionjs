---
title: 'API reference'
description: 'Pulse connection, publishing, subscriptions, errors, collections, and indexes'
---
...
```typescript
const subscription = await pulse.subscribeBatch(topic, batchHandler, {
  configVersion: 2,
  batchSize: 100,
  maxConcurrency: 2,
})
...
```typescript
type PulseBatchEventHandler<TTopic extends string, TData> = (
...
| `.deliveries` | `orionjs.pulse.deliveries` | Queue state, attempt lease, retry history, and outcome for one group and one or more events. |
