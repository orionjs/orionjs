...
| `collectionPrefix` | `string` | `orionjs.pulse` | Prefix for Pulse collections. |
...
| `pollIntervalMs` | `number` | `3000` | Idle interval for event discovery and work acquisition. |
...
| `lockTimeoutMs` | `number` | `30000` | Attempt lease duration. Heartbeats renew active leases. |
...
<Warning>
  `changeStreams` has been removed and cannot be enabled or disabled. Pulse always uses polling.
  Remove the field from existing JavaScript and TypeScript configurations;
...
| `maxConcurrency` | `number` | `workerCount` | Local concurrent handlers for this topic. |
...
Pulse projects the current attempt and up to the 10 most recent completed outcomes from the
delivery document. The delivery attempt counter remains exact even when older attempt details have
left that bounded window.
...
| `.subscriptions` | `orionjs.pulse.subscriptions` | Durable group/topic configuration, cursor, and discovery lease. |
| `.deliveries` | `orionjs.pulse.deliveries` | Queue state, attempt lease, retry history, and outcome for one group and event. |
...
| `pulse_deliveries_concurrent_pending` | `{consumerGroup: 1, nextAttemptAt: 1, createdAt: 1, topic: 1}` | Partial concurrent-pending queue |
| `pulse_deliveries_concurrent_processing` | `{consumerGroup: 1, lockedUntil: 1, topic: 1}` | Partial concurrent-processing queue |
