...
<Note>
  The current dashboard's direct delivery counters and attempt explorer describe execution version
  1. During an execution version 2 rollout, monitor `v2-*` delivery states through MongoDB Atlas or
  direct queries. The runtime `pulse.history.find()` API already projects recent version 2 attempts.
</Note>
