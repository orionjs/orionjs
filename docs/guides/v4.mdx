...
As of **v4.3**, the CLI defaults to **Bun** as the runtime. Node.js is still supported via the `--node` flag. See the [Using Bun with Orionjs](/guides/bun) guide for details.

- `bunx orion dev` to start in dev mode with Bun's built-in watch mode.
- `bunx orion prod` to start in prod mode (runs TypeScript directly with Bun).
- `bunx orion dev --node` / `bunx orion prod --node` to use Node.js instead.

You don't need to build the app for production anymore when using Bun.

> You must have installed `@orion-js/core` in your project.
...
We now recommend using **Vitest** instead of Jest for testing. Vitest provides better ESM support and faster test execution.

### Migration Steps

1. Replace Jest with Vitest:
```bash
pnpm remove jest @types/jest ts-jest @shelf/jest-mongodb
...
3. Update test setup file to use `MongoMemoryServer` directly instead of `@shelf/jest-mongodb`.

4. Replace `jest.fn()` with `vi.fn()` in your tests.

See the [Testing documentation](/testing) for complete setup instructions.
