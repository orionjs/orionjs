...
You can create custom validation rules using the `validate` option in the `@Prop()` decorator:
...
The options object provides:
- `doc`: The full document being validated
- `schema`: The current schema being validated against
