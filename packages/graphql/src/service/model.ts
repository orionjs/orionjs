...
const serviceMetadata = new WeakMap<any, { _serviceType: string, options: ModelResolversOptions, _typedSchema: any }>();
const modelResolversMetadata = new WeakMap<any, Record<string, any>>();

export function ModelResolvers(typedSchema: any,
  options: ModelResolversOptions = {},) {
  return function (target: any, context: ClassDecoratorContext<any>) {
    Service()(target, context);

    context.addInitializer(function (this) {
      serviceMetadata.set(this, { _serviceType: 'modelResolvers', options: options, _typedSchema: typedSchema });
    });
  };
}

export function ModelResolver<This, TArgs extends any[], TReturn extends any>(options?: Omit<ResolverOptions<any>, 'resolve' | 'middlewares'>) {
  return function (
...
export function getServiceModelResolvers(target: any): {
  ...
  if (!serviceMetadata.has(instance.constructor)) {
    throw new Error('You must pass a class decorated with @ModelResolvers to getServiceModelResolvers');
  }
